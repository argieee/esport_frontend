        ...
        <div className="p-8">
          <div className="grid grid-cols-3 gap-6 mb-8">
            <div className="bg-[#121a25] border border-slate-800 rounded-xl p-5 flex flex-col gap-1 items-center">
              <span className="text-xs text-slate-500 uppercase tracking-widest font-bold">Matches Won</span>
              <span className="text-4xl font-black text-white">{team.seriesWin !== undefined ? team.seriesWin : (team.w || 0)}</span>
            </div>
            <div className="bg-[#121a25] border border-slate-800 rounded-xl p-5 flex flex-col gap-1 items-center">
              <span className="text-xs text-slate-500 uppercase tracking-widest font-bold">Matches Lost</span>
              <span className="text-4xl font-black text-white">{team.seriesLoss !== undefined ? team.seriesLoss : (team.l || 0)}</span>
            </div>
            <div className="bg-[#121a25] border border-slate-800 rounded-xl p-5 flex flex-col gap-1 items-center">
              <span className="text-xs text-slate-500 uppercase tracking-widest font-bold">Set Diff</span>
              <span className="text-4xl font-black text-white">{team.setDiff !== undefined ? (team.setDiff > 0 ? '+'+team.setDiff : team.setDiff) : 0}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="px-6 py-5 border-b border-slate-800/60 flex items-center justify-between bg-slate-900/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold tracking-widest text-slate-500 uppercase mt-1">Rank #{player.rank || 1} — {activeGame}</p>
            </div>
          </div>
        ...
        <div className="p-6 grid grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="bg-[#151e2b] border border-[#232f40] rounded-xl p-5">
              <h3 className="text-xs font-black tracking-widest text-gray-500 uppercase mb-4">Combat Stats</h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">K/D Ratio</span><span className="text-lg font-black text-white">{player.kd}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">Total Kills</span><span className="text-lg font-black text-white">{player.kills}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">Total Deaths</span><span className="text-lg font-black text-white">{player.death || player.deaths}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">Rounds Played</span><span className="text-lg font-black text-white">{player.round || player.rounds}</span></div>
              </div>
            </div>
          </div>
          <div className="space-y-4">
            <div className="bg-[#151e2b] border border-[#232f40] rounded-xl p-5 h-full">
              <h3 className="text-xs font-black tracking-widest text-gray-500 uppercase mb-4">Advanced Metrics</h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">Kill per Round</span><span className="text-lg font-black text-[#8a9db8]">{player.kr}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">Survive Rate</span><span className="text-lg font-black text-[#8a9db8]">{player.sr}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">Headshot/Kill</span><span className="text-lg font-black text-[#8a9db8]">{player.hk}</span></div>
                <div className="flex justify-between items-center"><span className="text-sm font-bold text-slate-300">Avg Points</span><span className="text-lg font-black text-[#8a9db8]">{player.ap}</span></div>
              </div>
            </div>
          </div>
        </div>
...
const Leaderboards = ({ globalGame }) => {
  ...
  useEffect(() => {
     fetch('http://localhost:5000/api/match-records')
       .then(res => res.json())
  ...
  const mapAnalytics = useMemo(() => {
    ...
    const coldestMap = winRates[winRates.length - 1] ? { map: winRates[winRates.length - 1].map, banRate: 'N/A' } : null;
  ...
  }, [standings, h2hLeft, h2hRight]);
  const playerMatchup = useMemo(() => {
    const p1 = playerStats.find(p => p.ign === bestPlayerLeft) || topPlayers[0] || { kd: '0', kr: '0', hk: '0', sr: '0', ap: '0', kills: 0 };
    const p2 = playerStats.find(p => p.ign === bestPlayerRight) || topPlayers[1] || { kd: '0', kr: '0', hk: '0', sr: '0', ap: '0', kills: 0 };
    return {
  ...
  }, [playerStats, topPlayers, bestPlayerLeft, bestPlayerRight]);
              ...
              <div className="bg-[#121a25] rounded-xl border border-[#232f40] shadow-xl overflow-hidden flex flex-col">
                 <div className="grid grid-cols-[1fr_auto_1fr] items-stretch bg-[#151e2b] border-b border-[#232f40]">
                    <div className="py-5 px-8 flex justify-between items-center transition-colors duration-500" style={{ background: `linear-gradient(to right, ${theme.leftAccent}1A, transparent)` }}>
                      ...
                      <div className="w-12 h-12 rounded-full bg-[#1e2938] border-2 shadow-lg relative overflow-hidden flex items-center justify-center" style={{ borderColor: theme.leftAccent, boxShadow: `0 0 15px ${theme.leftAccent}4D` }}>
                      </div>
                    </div>
                    ...
                    <div className="py-5 px-8 flex justify-between items-center transition-colors duration-500" style={{ background: `linear-gradient(to left, ${theme.rightAccent}1A, transparent)` }}>
                      ...
                      <div className="flex flex-col text-right">
                         ...
                         <span className="font-black text-white text-lg tracking-wide">{playerMatchup.playerRight}</span>
                      </div>
                    </div>
                 </div>
              ...
              <div className="bg-[#121a25] rounded-xl border border-[#232f40] shadow-xl overflow-hidden relative group">
                ...
                <div className="flex justify-between items-center p-8 pb-4 relative z-10">
                  ...
                  <div className="flex flex-col items-center">
                    <span className="text-xs text-gray-400 font-black tracking-[0.3em] uppercase mb-1">Head to Head</span>
                  ...
                  {h2h.stats.map((stat, i) => {
                    ...
                    return (
                      <div key={i} className="flex flex-col space-y-3">
                        <div className="flex justify-between text-sm text-gray-400 font-semibold px-1">
                          <span style={{ color: theme.leftAccent }}>{stat.label}</span>
                          <span className="text-white bg-white/5 px-4 py-1 rounded-full text-xs border border-white/10 tracking-wider">
                            {stat.left} - {stat.right}
                          </span>
                          <span style={{ color: theme.rightAccent }}>{stat.label}</span>
                        </div>
                        <div className="h-8 w-full bg-[#0a0f16] rounded-md flex overflow-hidden border border-[#1e2938] shadow-inner relative">
                          <div className={`h-full bg-gradient-to-r ${theme.leftGradient} flex justify-end items-center px-4 text-sm font-bold text-white/90 transition-all duration-1000`} style={{ width: `${leftPercent}%` }}></div>
                          <div className="w-[2px] bg-[#1e2938] h-full z-10"></div>
                          <div className={`h-full bg-gradient-to-l ${theme.rightGradient} flex justify-start items-center px-4 text-sm font-bold text-white/90 transition-all duration-1000`} style={{ width: `${rightPercent}%` }}></div>
                        </div>
                      </div>
                    );
                  })}
             ...
             <div className="bg-[#121a25] rounded-xl border border-[#232f40] shadow-xl flex flex-col h-[500px] overflow-hidden">
               <div className="p-6 border-b border-[#232f40] bg-gradient-to-b from-[#151e2b] to-[#121a25]">
                 ...
                 <div className="relative group">
                   ...
                   <input type="text" placeholder="Search Player Here" className="w-full bg-[#0a0f16] border border-[#232f40] text-sm text-gray-200 rounded-lg py-3 pl-12 pr-4 focus:outline-none focus:border-[#406899] focus:ring-1 focus:ring-[#406899]/50 transition-all shadow-inner placeholder-gray-600" value={dailySearch} onChange={(e) => setDailySearch(e.target.value)} />
                 </div>
               </div>
