...
const PlayerRankings = ({ globalGame }) => {
  ...
  const liveValorantPlayers = useMemo(() => {
    ...
    return [...dbValorantStats].sort((a, b) => b.total_kills - a.total_kills).map((p, idx) => ({
      rank: idx + 1,
      name: p.ign,
      kills: p.total_kills,
      death: p.total_deaths,
      round: p.total_rounds_played,
      kd: p.total_deaths ? (p.total_kills / p.total_deaths).toFixed(2) : p.total_kills.toFixed(2),
      kr: p.total_rounds_played ? (p.total_kills / p.total_rounds_played).toFixed(2) : '0.00',
      sr: p.total_rounds_played ? ((p.total_rounds_played - p.total_deaths) / p.total_rounds_played).toFixed(2) : '0.00',
  ...
  const liveCrossfirePlayers = useMemo(() => {
    ...
    return [...dbCrossfireStats].sort((a, b) => b.total_kills - a.total_kills).map((p, idx) => ({
      rank: idx + 1,
      name: p.ign,
      kills: p.total_kills,
      death: p.total_deaths,
      ...
      kr: p.total_rounds_played ? (p.total_kills / p.total_rounds_played).toFixed(2) : '0.00',
      sr: p.total_rounds_played ? ((p.total_rounds_played - p.total_deaths) / p.total_rounds_played).toFixed(2) : '0.00',
      hk: p.total_kills ? (p.total_headshots / p.total_kills).toFixed(2) : '0.00',
      ap: p.performance_score ? Math.round(p.performance_score * 100) : 0
    }));
  }, [dbCrossfireStats]);
  const currentLivePlayers = activeGame === 'VALORANT' ? liveValorantPlayers : liveCrossfirePlayers;
  const valorantData = {
    dailyPlayers: [
  ...
  const aggregateMatchRecords = (records) => {
    ...
    records.forEach(r => {
      ...
      if (!ign) return;
      if (!playerStats[ign]) {
         playerStats[ign] = { ign, kills: 0, deaths: 0, rounds: 0, headshots: 0, matchCount: 0 };
      }
    ...
    return Object.values(playerStats).sort((a, b) => b.kills - a.kills).map((p, idx) => {
      const kd = p.deaths ? (p.kills / p.deaths).toFixed(2) : p.kills.toFixed(2);
      const kr = p.rounds ? (p.kills / p.rounds).toFixed(2) : '0.00';
      const sr = p.rounds ? ((p.rounds - p.deaths) / p.rounds).toFixed(2) : '0.00';
  ...
  const { displayDailyPlayers, displayWeeklyPlayers } = useMemo(() => {
    ...
    if (gameRecords.length === 0) return { displayDailyPlayers: [], displayWeeklyPlayers: [] };
    const latestMatch = gameRecords[0];
    const latestWeek = latestMatch.week;
    const latestDay = latestMatch.day;
    const dailyRecords = gameRecords.filter(r => r.week === latestWeek && r.day === latestDay);
  ...
  const filteredDaily = displayDailyPlayers.filter(p => p.name.toLowerCase().includes(dailySearch.toLowerCase()));
  const filteredWeekly = displayWeeklyPlayers.filter(p => p.name.toLowerCase().includes(weeklySearch.toLowerCase()));
  return (
    <div className="flex-1 bg-[#090e14] text-white overflow-y-auto flex flex-col h-full custom-scrollbar">
      ...
      `}</style>
      <div className="flex-1 overflow-y-auto custom-scrollbar px-8 lg:px-12 pb-16 pt-8 flex flex-col items-center h-full">
        <div className="w-full max-w-[1600px] h-full flex flex-col space-y-8">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 flex-1 pb-8 min-h-[600px]">
             {}
             <div className="bg-[#0b1018] rounded-2xl border border-slate-800 shadow-[0_10px_40px_rgba(0,0,0,0.6)] flex flex-col h-[450px] overflow-hidden relative group">
               ...
               <div className="p-5 border-b border-slate-800/80 bg-[#101722] relative z-10">
                 <div className="flex justify-between items-center mb-4">
                   <div className="flex items-center gap-3">
                     <div className="w-8 h-8 rounded-lg bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center">
                       <span className="text-cyan-400 font-black text-sm">D</span>
                     </div>
                     <h3 className="text-[13px] font-black text-white uppercase tracking-widest drop-shadow-md">Daily Player Ranking</h3>
                   </div>
                 </div>
                 <div className="relative group">
                   ...
                   <input 
                     ...
                     className="w-full bg-[#0a0f16]/80 border border-slate-700 text-sm text-white rounded-xl py-3 pl-11 pr-4 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/50 transition-all shadow-inner placeholder-gray-600 font-medium"
                     value={dailySearch}
                     onChange={(e) => setDailySearch(e.target.value)}
                   />
                 </div>
               </div>
               <div className="flex-1 overflow-y-auto custom-scrollbar bg-gradient-to-b from-[#0a0f16] to-[#0d131c] relative z-10">
                 <table className="w-full text-center text-[11px]">
                   <thead className="bg-[#121a25]/90 backdrop-blur-md sticky top-0 text-slate-500 font-black shadow-md z-10 text-[9px] tracking-[0.15em] uppercase border-b border-slate-800">
                     <tr>
                       <th className="py-4 px-2 text-left pl-6">Player Identity</th>
                   ...
                   <tbody className="divide-y divide-slate-800/50">
                     {filteredDaily.length > 0 ? filteredDaily.map((p, i) => (
                       <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer">
                         <td className="py-3 px-2 text-left pl-4 font-bold flex items-center space-x-4">
                           <div className={`w-6 h-6 flex items-center justify-center rounded-md font-black text-[10px] shadow-lg ${i === 0 ? 'bg-amber-500/20 text-amber-400 border border-amber-500/40' : i === 1 ? 'bg-slate-300/20 text-slate-300 border border-slate-300/40' : i === 2 ? 'bg-orange-700/20 text-orange-400 border border-orange-700/40' : 'bg-slate-800 text-slate-500'}`}>
                             {p.rank}
                           </div>
                           <span className="truncate max-w-[120px] text-white group-hover:text-cyan-400 transition-colors" title={p.name}>{p.name}</span>
                         </td>
                         <td className="py-3 px-1 text-slate-300 font-semibold">{p.kills}</td>
                         <td className="py-3 px-1 text-slate-400">{p.death}</td>
                         <td className="py-3 px-1 text-slate-500">{p.round}</td>
                         <td className="py-3 px-1 text-cyan-400 font-black">{p.kd}</td>
                         <td className="py-3 px-1 text-cyan-400 font-black">{p.kr}</td>
                         <td className="py-3 px-1 text-cyan-400 font-black">{p.sr}</td>
                         <td className="py-3 px-1 text-cyan-400 font-black">{p.hk}</td>
                       ...
                       <tr><td colSpan="9" className="py-12 text-slate-600 font-medium">No players found matching your search.</td></tr>
                     )}
                   </tbody>
                 </table>
               </div>
                 ...
                 <div className="flex justify-between items-center mb-4">
                   <div className="flex items-center gap-3">
                     <div className="w-8 h-8 rounded-lg bg-fuchsia-500/10 border border-fuchsia-500/20 flex items-center justify-center">
                       <span className="text-fuchsia-400 font-black text-sm">W</span>
                     </div>
                     <h3 className="text-[13px] font-black text-white uppercase tracking-widest drop-shadow-md">Weekly Player Ranking</h3>
                   </div>
                 </div>
                 <div className="relative group">
                   <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                   ...
                   <input 
                     type="text" 
                     placeholder="Search Player Here" 
                     className="w-full bg-[#0a0f16]/80 border border-slate-700 text-sm text-white rounded-xl py-3 pl-11 pr-4 focus:outline-none focus:border-fuchsia-500 focus:ring-1 focus:ring-fuchsia-500/50 transition-all shadow-inner placeholder-gray-600 font-medium"
                     value={weeklySearch}
                     ...
                     {filteredWeekly.length > 0 ? filteredWeekly.map((p, i) => (
                       <tr key={i} className="hover:bg-slate-800/30 transition-colors group cursor-pointer">
                         <td className="py-3 px-2 text-left pl-4 font-bold flex items-center space-x-4">
                           ...
                           <span className="truncate max-w-[120px] text-white group-hover:text-fuchsia-400 transition-colors" title={p.name}>{p.name}</span>
                         </td>
                         <td className="py-3 px-1 text-slate-300 font-semibold">{p.kills}</td>
                         <td className="py-3 px-1 text-slate-400">{p.death}</td>
                         <td className="py-3 px-1 text-slate-500">{p.round}</td>
                         <td className="py-3 px-1 text-fuchsia-400 font-black">{p.kd}</td>
                         <td className="py-3 px-1 text-fuchsia-400 font-black">{p.kr}</td>
                         <td className="py-3 px-1 text-fuchsia-400 font-black">{p.sr}</td>
                         <td className="py-3 px-1 text-fuchsia-400 font-black">{p.hk}</td>
                         <td className="py-3 px-1">
                           <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-1 rounded font-black text-[10px]">
                             {p.ap}
                           </span>
