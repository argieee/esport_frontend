...
const AuditLogs = () => {
  ...
  return (
    <div className="flex-1 bg-[#090e14] light:bg-[#f8fafc] text-white light:text-slate-900 overflow-y-auto flex flex-col h-full relative font-sans custom-scrollbar p-8">
      <style>{`
      ...
      <div className="max-w-[1400px] w-full mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-black text-white light:text-slate-900 uppercase tracking-widest mb-2 title-gradient">Audit Logs</h1>
            <p className="text-slate-400 light:text-slate-500 text-sm tracking-wide">System-wide record of data modifications and activities.</p>
          </div>
          <button 
            onClick={fetchLogs}
        ...
        <div className="bg-[#0f1722] light:bg-white rounded-xl border border-[#1c2532] light:border-slate-200 shadow-2xl light:shadow-sm flex flex-col overflow-hidden">
           <div className="p-5 border-b border-[#1c2532] light:border-slate-200 bg-[#121a25] light:bg-slate-50 flex justify-between items-center relative overflow-hidden">
               <div className="absolute top-0 left-0 w-1 h-full bg-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.8)]"></div>
               <h2 className="text-sm font-bold text-gray-300 light:text-slate-700 tracking-wide uppercase ml-3">Admin and User Activity Audit Logs (PH Context)</h2>
               <div className="flex gap-4">
                 <div className="flex items-center gap-2 px-3 py-1 bg-[#1a2533] light:bg-white rounded-lg border border-[#2a3648] light:border-slate-200 shadow-sm">
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-[#151e2b] light:bg-slate-100 text-gray-500 light:text-slate-500 border-b border-[#1c2532] light:border-slate-200">
                  <tr>
                    <th className="px-6 py-4 font-black uppercase tracking-widest text-[10px] w-40">Timestamp</th>
                    <th className="px-6 py-4 font-black uppercase tracking-widest text-[10px] w-48">Admin/System IGN</th>
                    <th className="px-6 py-4 font-black uppercase tracking-widest text-[10px] w-48">Action Type</th>
                    <th className="px-6 py-4 font-black uppercase tracking-widest text-[10px]">Description</th>
                    <th className="px-6 py-4 font-black uppercase tracking-widest text-[10px] w-24 text-center">Details</th>
                  </tr>
                </thead>
                          ...
                          <tr className={`hover:bg-[#1a2533]/50 light:hover:bg-slate-50 transition-colors group cursor-pointer ${isExpanded ? 'bg-[#151e2b] light:bg-slate-50' : ''}`}>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex flex-col items-start gap-2">
                                ...
                                {log.game && (
                                  <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border ${getGameColor(log.game)}`}>
                                    {log.game}
                                  </span>
                                )}
                              </div>
