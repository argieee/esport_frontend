...
const Card = ({ title, children, className = "" }) => (
  ...
  >
    ...
    <div className="flex-1 relative z-10 min-h-[300px]" style={{ padding: "16px" }}>{children}</div>
  </div>
);
                  ...
                  setTooltip({
                    ...
                    content: (
                      <div className="flex flex-col gap-1">
                        <div
                          className="font-black border-b border-gray-700 pb-1 mb-1 tracking-wider"
      ...
      >
        ...
        {agents.map((_, i) => (
          <line
            key={`hy-${i}`}
            x1={padX}
            y1={padY + (i * (h - padY - padBottom)) / (agents.length - 1 || 1)}
            x2={w - padX + 20}
            y2={padY + (i * (h - padY - padBottom)) / (agents.length - 1 || 1)}
            stroke="#1e2938"
          />
        ))}
        {maps.map((_, i) => (
          <line
            key={`hx-${i}`}
            x1={padX + (i * (w - padX * 2 - 20)) / (maps.length - 1 || 1)}
            y1={padY}
            x2={padX + (i * (w - padX * 2 - 20)) / (maps.length - 1 || 1)}
            y2={h - padBottom}
            stroke="#1e2938"
          />
        ))}
        ...
        {agents.map((agent, i) => {
          const y =
            padY + (i * (h - padY - padBottom)) / (agents.length - 1 || 1);
          return (
            <text
              key={`a-${i}`}
              x={padX - 10}
        ...
        {maps.map((m, i) => {
          ...
          return (
            <text
              ...
              fontSize="10"
              textAnchor="end"
              transform={`rotate(-45 ${x - 5} ${h - padBottom + 15})`}
              className="font-semibold"
            >
              {m}
            </text>
        ...
        {data.map((d, i) => {
          ...
          const y =
            padY + (aIdx * (h - padY - padBottom)) / (agents.length - 1 || 1);
          const x =
            padX + (mIdx * (w - padX * 2 - 20)) / (maps.length - 1 || 1);
...
const StrategyMap = ({
  ...
  const plantPct =
    totalEvents > 0 ? Math.round((plants / totalEvents) * 100) : 0;
  const defusePct =
    totalEvents > 0 ? Math.round((defuses / totalEvents) * 100) : 0;
  return (
    <div
...
const Analytics = ({ globalGame, globalTournament }) => {
    ...
    const fetchRecords = async () => {
      try {
        const res = await apiFetch(
          `/api/match-records?tournament=${encodeURIComponent(globalTournament || "Default")}`,
        );
        if (res.ok) {
  ...
  const uniqueMaps = useMemo(() => {
    ...
    const gameRecords = matchRecords.filter(
      (r) => r.game.toUpperCase() === gameUpper,
    );
    const maps = new Set();
  ...
  const openRadarOverview = (player) => {
    setModalData({
      ...
      description:
        "Comprehensive breakdown of player statistics across all calculated metrics. Displays the raw dataset being visualized.",
      data: player,
    });
  };
  const openEconOverview = (round) => {
    setModalData({
      ...
      description:
        "Deep dive into the financial decisions made during this specific round and how it impacted the overall win probability.",
      data: round,
    });
  };
  const openSynergyOverview = (synergy) => {
    setModalData({
      ...
      description:
        "Detailed metrics explaining the effectiveness of this specific agent pick on this map.",
      data: synergy,
    });
  };
  ...
  return (
    <div className="flex-1 bg-[#090e14] text-white overflow-y-auto flex flex-col h-full custom-scrollbar relative">
      ...
      {/* Main Dashboard Grid */}
      <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 flex flex-col items-center" style={{ padding: "32px" }}>
        <div className="w-full max-w-[1400px] grid grid-cols-1 xl:grid-cols-2 gap-8" style={{ paddingBottom: "64px" }}>
          <Card title="Global Player Skill Matrix">
            <RadarChart
              data={filteredRadar}
              axes={currentData.radarAxes}
