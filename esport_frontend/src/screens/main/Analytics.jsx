  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center p-2 cursor-pointer group" onClick={() => onSelect({ type: 'Heatmap', area: selectedMap || 'No Data', details: 'Strategy heatmap based on match submissions.' })}>
       ...
       <div className="relative w-full max-w-[800px] aspect-[4/3] bg-[#0a0f16] rounded-xl border border-[#2a3648] overflow-hidden shadow-2xl flex items-center justify-center group-hover:border-blue-500 transition-all">
          <img 
             src={selectedMap ? `/assets/${selectedMap.replace(/ /g, '_')}.png` : '/assets/map_placeholder.png'} 
             onError={(e) => { e.target.onerror = null; e.target.src = '/assets/map_placeholder.png'; }}
          ...
          {mapEvents.map(h => (
            <div 
              key={h.id} 
              className={`absolute w-4 h-4 -ml-2 -mt-2 rounded-full border-2 border-white animate-in zoom-in hover:scale-150 transition-transform cursor-help z-30 ${h.event_type === 'Plant' ? 'bg-red-500 shadow-[0_0_12px_#ef4444]' : 'bg-blue-500 shadow-[0_0_12px_#3b82f6]'}`}
                ...
                setTooltip({
                  ...
                  content: (
                    <div className="flex flex-col gap-1 min-w-[80px]">
                      <div className={`font-black uppercase tracking-widest text-xs border-b pb-1 ${h.event_type === 'Plant' ? 'text-red-400 border-red-900/50' : 'text-blue-400 border-blue-900/50'}`}>{h.event_type}</div>
                      <div className="text-gray-300 font-bold">Round <span className="text-white">{h.round_number}</span></div>
                    </div>
                  )
                });
              }}
       ...
       {}
       <div className="absolute bottom-4 right-4 bg-[#0a0f16]/90 p-3 rounded-lg border border-[#232f40] flex flex-col items-start backdrop-blur-md pointer-events-none shadow-xl min-w-[140px]">
         <span className="text-[10px] font-bold text-white mb-3 uppercase tracking-widest">Legend</span>
         <div className="flex items-center justify-between w-full mb-2">
            <div className="flex items-center gap-3">
               <div className="w-3 h-3 rounded-full bg-red-500 shadow-[0_0_8px_#ef4444] border border-white shrink-0"></div>
               <span className="text-[10px] font-bold text-slate-300 uppercase">Plant</span>
            </div>
            <span className="text-[10px] font-bold text-slate-400">{plantPct}%</span>
         </div>
         <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-3">
               <div className="w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_8px_#3b82f6] border border-white shrink-0"></div>
               <span className="text-[10px] font-bold text-slate-300 uppercase">Defuse</span>
            </div>
            <span className="text-[10px] font-bold text-slate-400">{defusePct}%</span>
         </div>
       </div>
    </div>
  );
...
const Analytics = ({ globalGame, globalTournament }) => {  const [activeGame, setActiveGame] = useState('VALORANT');  const [dataType, setDataType] = useState('ALL PLAYERS');  const [searchQuery, setSearchQuery] = useState('');  const [modalData, setModalData] = useState(null);
  const [modalTooltip, setModalTooltip] = useState({ visible: false, x: 0, y: 0, content: null });
  ...
  // --- MOCK DATA ---  const valorantTheme = {    color1: '#ff3366', // Red    color2: '#00d0eb', // Cyan    color3: '#a78bfa', // Purple  };  const crossfireTheme = {    color1: '#4c7fd6', // Blue    color2: '#f59e0b', // Amber    color3: '#10b981', // Green  };  const valData = {    theme: valorantTheme,    radarAxes: ['Agent Proficiency', 'Reaction Time', 'Econ Management', 'Headshot %', 'Utility Usage', 'Clutch %'],    radar: [      { name: 'Elite Player', color: '#00d0eb', stats: [90, 85, 75, 80, 85, 70] },      { name: 'Pro Player', color: '#ff3366', stats: [75, 95, 60, 90, 65, 85] },      { name: 'Average Player', color: '#a78bfa', stats: [50, 60, 45, 55, 40, 50] }    ],    econ: [      { round: 1, spent: 800, remaining: 150, prob: 45 },      { round: 2, spent: 1500, remaining: 1000, prob: 52 },      { round: 3, spent: 3900, remaining: 200, prob: 60 },      { round: 4, spent: 4200, remaining: 500, prob: 65 },      { round: 5, spent: 3000, remaining: 2000, prob: 58 },      { round: 6, spent: 4500, remaining: 1500, prob: 70 },      { round: 7, spent: 5000, remaining: 3000, prob: 75 },      { round: 8, spent: 4800, remaining: 4500, prob: 82 },      { round: 9, spent: 5500, remaining: 6000, prob: 90 },    ],    synergy: [      { agent: 'Jett', map: 'Haven', pickRate: 280, winRate: 52 },      { agent: 'Omen', map: 'Haven', pickRate: 220, winRate: 65 },      { agent: 'Sova', map: 'Ascent', pickRate: 290, winRate: 72 },      { agent: 'Killjoy', map: 'Ascent', pickRate: 250, winRate: 68 },      { agent: 'Viper', map: 'Breeze', pickRate: 260, winRate: 75 },      { agent: 'Raze', map: 'Split', pickRate: 240, winRate: 58 },      { agent: 'Breach', map: 'Fracture', pickRate: 180, winRate: 62 },      { agent: 'Cypher', map: 'Split', pickRate: 150, winRate: 48 },      { agent: 'Phoenix', map: 'Haven', pickRate: 120, winRate: 45 },    ]  };  const cfData = {    theme: crossfireTheme,    radarAxes: ['Weapon Mastery', 'Reaction Time', 'Positioning', 'Headshot %', 'Utility Usage', 'Clutch %'],    radar: [      { name: 'CF Elite', color: '#f59e0b', stats: [95, 90, 85, 88, 60, 80] },      { name: 'CF Pro', color: '#4c7fd6', stats: [80, 95, 70, 92, 50, 85] },      { name: 'Average Player', color: '#10b981', stats: [60, 55, 50, 45, 30, 40] }    ],    econ: [      { round: 1, spent: 500, remaining: 200, prob: 50 },      { round: 2, spent: 1200, remaining: 800, prob: 55 },      { round: 3, spent: 2500, remaining: 1000, prob: 62 },      { round: 4, spent: 3000, remaining: 1500, prob: 68 },      { round: 5, spent: 2800, remaining: 2500, prob: 64 },      { round: 6, spent: 3500, remaining: 3000, prob: 72 },      { round: 7, spent: 4000, remaining: 4000, prob: 78 },      { round: 8, spent: 4500, remaining: 5000, prob: 85 },      { round: 9, spent: 5000, remaining: 7000, prob: 95 },    ],    synergy: [      { agent: 'Sniper', map: 'Black Widow', pickRate: 270, winRate: 70 },      { agent: 'Rifler', map: 'Black Widow', pickRate: 290, winRate: 55 },      { agent: 'Assault', map: 'Port', pickRate: 250, winRate: 60 },      { agent: 'Support', map: 'Port', pickRate: 180, winRate: 65 },      { agent: 'Sniper', map: 'Eagle Eye', pickRate: 240, winRate: 75 },      { agent: 'Rifler', map: 'Eagle Eye', pickRate: 260, winRate: 58 },      { agent: 'Assault', map: 'Sub Base', pickRate: 210, winRate: 62 },      { agent: 'Support', map: 'Sub Base', pickRate: 150, winRate: 48 },      { agent: 'Sniper', map: 'Ankara', pickRate: 190, winRate: 50 },    ]  };  const currentData = activeGame === 'VALORANT' ? valData : cfData;  // Filter Data based on Search Query  const sq = searchQuery.toLowerCase();  const filteredRadar = currentData.radar.filter(p => p.name.toLowerCase().includes(sq));  const filteredSynergy = currentData.synergy.filter(s => s.agent.toLowerCase().includes(sq) || s.map.toLowerCase().includes(sq));  // Modal Handlers  const openRadarOverview = (player) => {    setModalData({      title: `${player.name} Skill Overview`,      description: 'Comprehensive breakdown of player statistics across all calculated metrics. Displays the raw dataset being visualized.',      data: player    });  };  const openEconOverview = (round) => {    setModalData({      title: `Round ${round.round} Economic Analysis`,      description: 'Deep dive into the financial decisions made during this specific round and how it impacted the overall win probability.',      data: round    });  };  const openSynergyOverview = (synergy) => {    setModalData({      title: `${synergy.agent} on ${synergy.map} Synergy`,      description: 'Detailed metrics explaining the effectiveness of this specific agent pick on this map.',      data: synergy    });  };  const openHeatmapOverview = (heatmap) => {    setModalData({      title: `Heatmap: ${heatmap.area}`,      description: heatmap.details,      data: heatmap    });  };  return (    <div className="flex-1 bg-[#090e14] text-white overflow-y-auto flex flex-col h-full custom-scrollbar relative">      <style>{`        .custom-scrollbar::-webkit-scrollbar { width: 5px; }        .custom-scrollbar::-webkit-scrollbar-track { background: transparent; }        .custom-scrollbar::-webkit-scrollbar-thumb { background: #1e293b; border-radius: 9999px; }        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #334155; }      `}</style>      {}      <div className="bg-[#0f1722]/80 backdrop-blur-xl border-b border-[#1c2532] shadow-xl py-4 px-8 relative z-30 flex flex-col items-center">        <div className="w-full max-w-[1400px] flex items-center justify-between">          {}          <div className="flex space-x-12">            <div className="flex flex-col">              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Game:</label>              <div className="relative group cursor-pointer">                <select                   className="appearance-none bg-[#121a25] border border-[#2a3648] text-white font-bold tracking-wider rounded-lg py-2 pl-4 pr-10 focus:outline-none focus:border-[#406899] hover:bg-[#1a2533] transition-colors shadow-inner"                  value={activeGame}                  onChange={(e) => {                    setActiveGame(e.target.value);                    setSearchQuery(''); // Reset search on game change                  }}                >                  <option value="VALORANT">VALORANT</option>                  <option value="CROSSFIRE">CROSSFIRE</option>                </select>                <div className="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none">                  <ChevronDown />                </div>              </div>            </div>            <div className="flex flex-col">              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Data Type:</label>              <div className="relative group cursor-pointer">                <select                   className="appearance-none bg-[#121a25] border border-[#2a3648] text-white font-bold tracking-wider rounded-lg py-2 pl-4 pr-10 focus:outline-none focus:border-[#406899] hover:bg-[#1a2533] transition-colors shadow-inner"                  value={dataType}                  onChange={(e) => setDataType(e.target.value)}                >                  <option value="ALL PLAYERS">ALL PLAYERS</option>                  <option value="PRO PLAYERS">PRO PLAYERS</option>                  <option value="CASUAL">CASUAL</option>                </select>                <div className="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none">                  <ChevronDown />                </div>              </div>            </div>          </div>          {/* Search - Connected to State */}          <div className="relative w-80">            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">              <SearchIcon />            </div>            <input               type="text"               placeholder="Filter by Player, Agent, or Map..."               value={searchQuery}              onChange={(e) => setSearchQuery(e.target.value)}              className="w-full bg-[#e2e8f0] text-gray-900 font-medium rounded-full py-2.5 pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-inner transition-all placeholder-gray-500"            />          </div>        </div>      </div>      {/* Main Dashboard Grid */}      <div className="flex-1 overflow-y-auto custom-scrollbar p-8 relative z-10 flex flex-col items-center">        <div className="w-full max-w-[1400px] grid grid-cols-1 xl:grid-cols-2 gap-8 pb-16">          <Card title="Global Player Skill Matrix">             <RadarChart data={filteredRadar} axes={currentData.radarAxes} theme={currentData.theme} onSelect={openRadarOverview} />          </Card>          <Card title="Economic Efficiency over Rounds">             <EconChart data={currentData.econ} theme={currentData.theme} onSelect={openEconOverview} />          </Card>          <Card title="Agent/Map Synergies" className="min-h-[350px]">             <BubbleChart data={filteredSynergy} theme={currentData.theme} onSelect={openSynergyOverview} />          </Card>          <Card title={`${activeGame === 'VALORANT' ? 'Valorant' : 'Crossfire'} Team Round-Start Strategy Heatmap`} className="min-h-[350px]">             <StrategyMap theme={currentData.theme} onSelect={openHeatmapOverview} availableMaps={uniqueMaps} heatmapEvents={heatmapEvents} activeGame={activeGame} />          </Card>        </div>      </div>      {}
      {modalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setModalData(null)}>
          <div className={`bg-[#121a25] border border-[#232f40] rounded-xl p-8 w-full shadow-2xl transform transition-all scale-100 opacity-100 ${modalData.data?.type === 'Heatmap' ? 'max-w-4xl' : 'max-w-md'}`} onClick={e => e.stopPropagation()}>
            ...
            {modalData.data?.type === 'Heatmap' ? (
              <div className="relative w-full aspect-[4/3] bg-[#0a0f16] rounded-xl border border-[#2a3648] overflow-hidden shadow-inner flex items-center justify-center mt-4">
                 <img 
                    src={modalData.data.area && modalData.data.area !== 'No Data' ? `/assets/${modalData.data.area.replace(/ /g, '_')}.png` : '/assets/map_placeholder.png'} 
                    onError={(e) => { e.target.onerror = null; e.target.src = '/assets/map_placeholder.png'; }}
                    className="absolute inset-0 w-full h-full object-contain opacity-90" 
                    alt="Map Full"
                 />
                 {heatmapEvents && heatmapEvents.filter(h => h.match_id && h.match_id.includes(modalData.data.area) && h.game && h.game.toUpperCase() === activeGame.toUpperCase()).map(h => (
                    <div 
                      key={h.id} 
                      className={`absolute w-5 h-5 -ml-2.5 -mt-2.5 rounded-full border-[3px] border-white hover:scale-150 transition-transform cursor-help z-30 ${h.event_type === 'Plant' ? 'bg-red-500 shadow-[0_0_15px_#ef4444]' : 'bg-blue-500 shadow-[0_0_15px_#3b82f6]'}`}
                      style={{ left: `${h.coord_x}%`, top: `${h.coord_y}%` }}
                      onMouseMove={(e) => {
                        e.stopPropagation();
                        const rect = e.target.parentElement.getBoundingClientRect();
                 ...
                 <div className="absolute bottom-6 right-6 bg-[#0a0f16]/90 p-4 rounded-xl border border-[#232f40] flex flex-col items-start backdrop-blur-md shadow-2xl">
                   <span className="text-xs font-black text-white mb-4 uppercase tracking-widest">Legend</span>
                   <div className="flex items-center gap-4 mb-3">
                      <div className="w-4 h-4 rounded-full bg-red-500 shadow-[0_0_10px_#ef4444] border-2 border-white shrink-0"></div>
                      <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Plant</span>
                   </div>
                   <div className="flex items-center gap-4">
                      <div className="w-4 h-4 rounded-full bg-blue-500 shadow-[0_0_10px_#3b82f6] border-2 border-white shrink-0"></div>
                      <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Defuse</span>
                   </div>
                 </div>
              </div>
            ) : (
