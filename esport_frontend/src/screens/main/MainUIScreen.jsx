...
const MainUIScreen = ({ onLogout }) => {
  ...
  const [themeHue, setThemeHue] = useState(() => localStorage.getItem('themeHue') || '0');
  ...
  useEffect(() => {
    localStorage.setItem('themeHue', themeHue);
    document.documentElement.style.setProperty('--theme-hue', `${themeHue}deg`);
  }, [themeHue]);
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      <aside 
        className={`bg-[#0b1120] light:bg-white border-slate-800/50 light:border-slate-200 z-20 shrink-0 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] select-none
        ...
        <div className="w-full h-full flex flex-col font-sans select-none shrink-0 pb-6 pt-14">
          ...
          <div className="shrink-0 flex items-center animate-fade-in-down group cursor-pointer" style={{ padding: "32px 32px 16px 32px" }}>
            <div className="flex items-center gap-4 transition-transform duration-500 ease-out group-hover:translate-x-2">
              ...
              <div className="flex items-center justify-center min-w-[48px] h-12 rounded-full bg-cyan-950/40 light:bg-cyan-100 border border-cyan-500/40 light:border-cyan-300 shadow-[0_0_20px_rgba(6,182,212,0.3)] light:shadow-sm relative overflow-hidden shrink-0 transition-all duration-500 group-hover:shadow-[0_0_30px_rgba(34,211,238,0.6)] group-hover:scale-110 group-hover:border-cyan-400/60">
                 <img src="/logo.png" alt="Logo" className="w-full h-full object-cover relative z-10 transition-transform duration-500 group-hover:scale-110" onError={(e) => { e.target.style.display='none' }} />
                 <span className="absolute text-cyan-400 light:text-cyan-600 font-black text-sm tracking-widest drop-shadow-md z-0 transition-transform duration-500 group-hover:scale-110">ES</span>
              </div>
              <h1 className="text-2xl font-black text-white light:text-slate-900 tracking-[0.15em] drop-shadow-lg transition-colors duration-500 group-hover:text-cyan-400">
                ESPORTS
              </h1>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden flex flex-col custom-scrollbar mt-2" style={{ padding: "0 32px 32px 32px" }}>
            ...
            {sidebarData.map((section, idx) => (
              ...
              >
                ...
                <h2 className="text-[11px] font-bold text-slate-500 tracking-[0.25em] uppercase mb-4 px-2">
                  {section.category}
                </h2>
                    ...
                    return (
                      <li key={link.name}>
                        <button
                          ...
                          className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all duration-300 ease-out transform ${
                            isActive 
                              ? 'bg-gradient-to-r from-cyan-900/50 to-cyan-900/10 light:from-cyan-100 light:to-white text-cyan-400 light:text-cyan-600 font-bold border border-cyan-500/30 light:border-cyan-300 shadow-[inset_0_0_20px_rgba(0,240,255,0.1)] light:shadow-sm translate-x-1' 
                              : 'text-slate-400 light:text-slate-600 hover:bg-slate-800/40 light:hover:bg-slate-100 hover:text-slate-200 light:hover:text-slate-900 font-medium hover:translate-x-1'
                          }`}
                        >
                          {IconComponent && (
                            <IconComponent 
                              ...
                              className={`transition-colors duration-300 ${isActive ? 'text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]' : 'text-slate-500'}`} 
                            />
                          )}
            ...
            <button 
              ...
              className="group w-full flex items-center gap-4 px-4 py-3 rounded-xl text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-all duration-300 transform hover:-translate-y-1"
            >
              <LogOut size={20} strokeWidth={2} className="text-slate-400 group-hover:text-red-500 transition-colors" />
      ...
      {/* MAIN CONTENT AREA */}
      <main className="flex-1 bg-[#040814] light:bg-[#f8fafc] flex flex-col overflow-hidden relative">
        {/* Modern Glassmorphism Header */}
        <header className="h-24 border-b border-slate-800/50 light:border-slate-200 flex items-center justify-between px-8 md:px-10 bg-[#0B1120]/80 light:bg-white/80 backdrop-blur-xl z-10 shrink-0 shadow-sm transition-all duration-500">
          <div className="flex items-center gap-10"> 
             {/* Sidebar Toggle Button */}
             <button 
                onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                className="bg-white/5 border border-white/10 text-slate-400 hover:text-white hover:bg-white/10 hover:shadow-[0_0_15px_rgba(255,255,255,0.1)] transition-all duration-300 group flex items-center justify-center shrink-0"
             ...
             <div className="flex items-center gap-6">
                ...
                <h2 className="text-xl md:text-2xl font-black text-white light:text-slate-900 tracking-[0.2em] uppercase drop-shadow-sm">
                  {activeTab}
                </h2>
             </div>
             ...
             <select 
               ...
               className="bg-slate-800 light:bg-white text-white light:text-slate-900 rounded-xl px-4 py-2 outline-none border border-slate-700/50 light:border-slate-300 hover:border-[#00ffcc]/50 transition-colors cursor-pointer ml-6"
             >
          ...
          <div className="flex items-center gap-4 text-slate-400">
             <button 
                ...
                className="p-3 rounded-2xl bg-transparent hover:bg-white/5 text-slate-400 hover:text-white transition-colors border border-transparent hover:border-white/10"
             >
                {theme === 'dark' ? (
        ...
        <div className="flex-1 overflow-auto relative z-10 bg-[#040814] light:bg-[#f8fafc]">
            {renderContent()}
        </div>
      </main>
      {/* Create Folder Modal */}
